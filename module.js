...
function run$2(config2) {
  var _a2;
  const { editor, from: from2, to, text: text2, rules: rules2, plugin } = config2;
  const { view } = editor;
  if (view.composing) {
    return false;
  }
...
function init$1({
  ...
  const qsNonAccepting = [APOSTROPHE$1, COLON, COMMA, DOT, EXCLAMATION, PERCENT, QUERY, QUOTE, SEMI, OPENANGLEBRACKET, CLOSEANGLEBRACKET, OPENBRACE, CLOSEBRACE, CLOSEBRACKET, OPENBRACKET, OPENPAREN, CLOSEPAREN, FULLWIDTHLEFTPAREN, FULLWIDTHRIGHTPAREN, LEFTCORNERBRACKET, RIGHTCORNERBRACKET, LEFTWHITECORNERBRACKET, RIGHTWHITECORNERBRACKET, FULLWIDTHLESSTHAN, FULLWIDTHGREATERTHAN];
  const localpartAccepting = [AMPERSAND, APOSTROPHE$1, ASTERISK, BACKSLASH, BACKTICK, CARET, DOLLAR, EQUALS, HYPHEN, OPENBRACE, CLOSEBRACE, PERCENT, PIPE, PLUS, POUND, QUERY, SLASH, SYM, TILDE, UNDERSCORE];
  ...
  const DomainDotTld = makeState(Url$1);
  ...
  const DomainDotTldColonPort = makeState(Url$1);
  ...
  for (let i = 0; i < bracketPairs.length; i++) {
    ...
    const UrlOpen = tt(Url$1$1, OPEN);
    tt(UrlNonaccept, OPEN, UrlOpen);
    tt(UrlOpen, CLOSE, Url$1$1);
    const UrlOpenQ = makeState(Url$1);
    ...
    tt(UrlOpenQ, CLOSE, Url$1$1);
    tt(UrlOpenSyms, CLOSE, Url$1$1);
  }
...
const CharacterCount = Extension.create({
  ...
  addProseMirrorPlugins() {
    let initialEvaluationDone = false;
    const getLimit = () => {
      return typeof this.options.limit === "function" ? this.options.limit() : this.options.limit;
    };
    return [
      new Plugin({
        key: new PluginKey("characterCount"),
...
const manageFormatsCleanup = (editor, position, tr2, prev, options) => {
  const rule = options.rules.find(({ id }) => id === prev.ruleId);
  if (!rule) {
    return;
  }
  const mark = rule.getMarkType(editor);
  if (position > prev.to) {
    tr2.removeMark(prev.to, position, mark);
    return;
  }
  tr2.removeMark(
    Math.max(position - 1, 0),
    Math.min(prev.to + 1, tr2.doc.content.size - 1),
    mark
  );
};
const manageFormatsApply = (editor, state, tr2, next2, options) => {
  const rule = options.rules.find(({ id }) => id === next2.ruleId);
  if (!rule) {
    return;
  }
  const findAppliedMark = (pos) => {
    const posData = state.doc.resolve(pos);
    return posData.marks().find(({ type }) => type === mark);
  };
  const mark = rule.getMarkType(editor);
  const appliedMark = findAppliedMark(next2.from) ?? findAppliedMark(next2.from + 1);
  if (!appliedMark) {
    return;
  }
  tr2.removeMark(next2.from, next2.to, mark);
  tr2.addMark(next2.from, next2.to, appliedMark);
};
const SpecialWordSuggestion = Extension.create({
  ...
  addCommands() {
    return {
      applyDetectedSpecialWord: (payload) => ({ state, dispatch }) => {
        const rule = this.options.rules.find((r) => r.id === payload.ruleId);
        if (!rule) return false;
        const markType = rule.getMarkType(this.editor);
        if (!markType) return false;
        const head = state.selection.head ?? state.selection.from;
        const found2 = rule.getMatchData(state.doc, head);
    ...
    return [
      new Plugin({
        appendTransaction: (transactions, _oldState, newState) => {
          ...
          const hasChanges = transactions.some(({ docChanged }) => docChanged);
          const tr2 = newState.tr;
          const head = newState.selection.head ?? newState.selection.from;
          const prev = this.storage.active;
          let next2 = null;
          for (const rule of this.options.rules) {
            const m = rule.getMatchData(newState.doc, head);
            if (!m) continue;
            next2 = m;
            break;
          }
          if (next2 && head >= next2.from && head <= next2.to) {
            manageFormatsApply(this.editor, newState, tr2, next2, this.options);
            if (hasChanges) {
              this.options.onDetect?.(next2);
            }
            this.storage.active = next2;
            this.storage.position = head;
          } else if (prev) {
            if (hasChanges) {
              manageFormatsCleanup(this.editor, head, tr2, prev, this.options);
            }
            this.options.onDetect?.({ ...prev, body: "" });
            this.storage.active = null;
            this.storage.position = null;
          }
          return tr2.steps.length || tr2.selectionSet ? tr2 : null;
        }
      })
    ];
  }
});
const MENTION_URL_PREFIX = "setka://account/";
const WORD_CHAR = /^[\p{L}\p{M}\p{N}_-]$/u;
const MENTION_PATTERN = /[\p{L}]+(?: [\p{L}]+)?/gu;
const getBlockCtx = (doc2, pos) => {
  const $pos = doc2.resolve(pos);
  const parent = $pos.parent;
  if (!parent?.isTextblock) return null;
  const blockFrom = $pos.start();
  const blockTo = $pos.end();
  const text2 = doc2.textBetween(blockFrom, blockTo, "\n", "\n");
  const caretInBlock = pos - blockFrom;
  return { text: text2, blockFrom, blockTo, caretInBlock };
};
const clampDocPos = (doc2, p) => Math.max(0, Math.min(p, doc2.content.size));
const scanTokenInBlock = (text2, caret, trigger) => {
  const i = Math.max(0, Math.min(text2.length, caret));
  const leftStart = Math.max(0, i - 1);
  let left = leftStart;
  while (left > 0 && WORD_CHAR.test(text2[left])) left--;
  if (text2[left] !== trigger) return null;
  let right = left + 1;
  while (right < text2.length && WORD_CHAR.test(text2[right])) right++;
  const body = text2.slice(left, right);
  if (!body) return null;
  return { relFrom: left, relTo: right, body };
};
const scanLinkedTokenInBloc = (doc2, pos, hrefPrefix, checkingPatter) => {
  const ctx = getBlockCtx(doc2, pos);
  if (!ctx) return null;
  const { blockFrom, blockTo } = ctx;
  const hasPrefixedAt = (p) => {
    const $p = doc2.resolve(clampDocPos(doc2, p));
    return $p.marks().some(
      (m) => typeof m.attrs?.href === "string" && m.attrs.href.startsWith(hrefPrefix)
    );
  };
  if (!hasPrefixedAt(pos)) {
    if (hasPrefixedAt(pos - 1)) {
      pos -= 1;
    } else if (hasPrefixedAt(pos + 1)) {
      pos += 1;
    } else {
      return null;
    }
  ...
  let from2 = pos;
  while (from2 >= blockFrom && hasPrefixedAt(from2)) from2--;
  const matching = doc2.textBetween(from2, blockTo, "\n", "\n").match(checkingPatter);
  const matchingWord = matching?.[0];
  if (!matchingWord) {
    return null;
  }
  const to = from2 + matchingWord.length;
  const body = doc2.textBetween(from2, to, "\n", "\n");
  return {
    relFrom: from2 - blockFrom,
    relTo: to - blockFrom,
    body
  };
};
const hashtagRule = {
  id: "hashtag",
  getMarkType: (editor) => editor?.schema.marks.link ?? null,
  getMatchData(doc2, position) {
    const ctx = getBlockCtx(doc2, position);
    if (!ctx) return null;
    const { text: text2, blockFrom, caretInBlock } = ctx;
    const res = scanTokenInBlock(
      text2,
      caretInBlock,
      "#"
      /* Hashtag */
    ...
    if (!res) return null;
    const { relFrom, relTo, body } = res;
    const from2 = blockFrom + relFrom;
    const to = blockFrom + relTo;
    return { from: from2, to, body, ruleId: this.id };
  }
};
const mentionRule = {
  id: "mention",
  ...
  getMatchData(doc2, position) {
    ...
    if (!ctx) return null;
    const { text: text2, blockFrom, caretInBlock } = ctx;
    const res = scanLinkedTokenInBloc(
      doc2,
      caretInBlock,
      MENTION_URL_PREFIX,
      MENTION_PATTERN
...
function findUnescaped(state, needle, from2) {
  const { src, posMax } = state;
  let i = from2;
  const nlen = needle.length;
  while (i + nlen <= posMax) {
    const at = src.indexOf(needle, i);
    if (at === -1 || at + nlen > posMax) return -1;
    let backslashes = 0;
    let j = at - 1;
    ...
    if ((backslashes & 1) === 0) return at;
    i = at + 1;
  }
  return -1;
}
function makeDelimitedRule(options) {
  const { name, marker, tag } = options;
  const firstCh = marker.charCodeAt(0);
  return function(state, silent) {
    const { src, pos, posMax } = state;
    if (pos + marker.length > posMax) return false;
    if (src.charCodeAt(pos) !== firstCh) return false;
    if (src.slice(pos, pos + marker.length) !== marker) return false;
    const after = pos + marker.length;
    if (after >= posMax) return false;
    const close2 = findUnescaped(state, marker, after);
    if (close2 === -1) return false;
    if (!silent) {
      ...
      state.pos = close2 + marker.length;
...
class MarkdownService {
  ...
  constructor() {
    ...
    this.overrideMarkdownRules(this.md);
    this.td = new TurndownService();
    this.overrideTurndownRules(this.td);
  }
  parse(content) {
    return this.md.renderInline(content);
  }
  serialize(content, extensions) {
    if (!content) {
      return "";
    }
    return this.td.turndown(generateHTML(content.toJSON(), extensions ?? []));
  }
  overrideMarkdownRules(markdownIt) {
    markdownIt.inline.ruler.before(
      "text",
      "underline_delim",
      makeDelimitedRule({ name: "u", marker: "++", tag: "u" })
    );
    markdownIt.inline.ruler.before(
      "text",
      "italic_delim",
      makeDelimitedRule({ name: "em", marker: "_", tag: "em" })
    );
  }
  overrideTurndownRules(turndown) {
    TurndownService.prototype.escape = (string) => {
      ...
      return escapes2.reduce((accumulator, escape2) => {
        const [escaping, escaped] = escape2;
        return accumulator.replace(escaping, escaped);
      }, string);
    };
    ...
    turndown.addRule("underlineStyle", {
      filter: (node) => {
        if (!(node instanceof HTMLElement)) return false;
        const style2 = node.getAttribute("style") || "";
        return /text-decoration\s*:\s*underline/i.test(style2);
      },
      replacement: (content, node) => {
        if (node.parentElement?.nodeName === "U") return content;
        return `++${content}++`;
      }
    });
    turndown.addRule("italicTagAsUnderscore", {
...
class TiptapAdapter {
  ...
  init(container) {
    this.editor = new Editor({
      ...
      extensions: [
        ...
        Underline,
        Strike,
        index_default$2.extend({ inclusive: false }).configure({
          openOnClick: false,
          protocols: ["http", "https", "setka"]
        }),
        SpecialWordSuggestion.configure({
          rules: [hashtagRule, mentionRule],
          onDetect: (m) => {
            switch (m.ruleId) {
              case "hashtag": {
                this.emit("hashtagSuggested", m.body);
                break;
              }
              case "mention": {
                this.emit("mentionSuggested", m.body);
                break;
              }
            }
          }
        ...
        CharacterCount.configure({
          limit: () => this.limit,
          textCounter: (text2) => [...new Intl.Segmenter().segment(text2)].length,
          onChange: (count) => {
  ...
  deinit() {
    ...
    this.unwireEditorEvents(this.editor);
    this.editor.destroy();
    this.editor = null;
  ...
  applyLink(href) {
    ...
    const { from: from2, empty: empty2 } = state.selection;
    const linkType = state.schema.marks.link;
    const linkRange = empty2 && linkType ? getMarkRange(state.doc.resolve(from2), linkType) : null;
    if (!href) {
      if (linkRange) {
    ...
    if (linkRange) {
      chain.insertContentAt(linkRange, href).setTextSelection({
        from: linkRange.from,
        to: linkRange.from + href.length
      }).setLink({ href }).setTextSelection(linkRange.from + href.length).run();
      return;
    }
    if (empty2) {
      chain.insertContent(href);
      const sel = this.editor.state.selection;
      const start = sel.head ?? sel.from;
      chain.setTextSelection({ from: start, to: start + href.length }).setLink({ href }).setTextSelection(start + href.length).run();
      return;
    }
  ...
  getContent() {
    ...
    return this.markdown.serialize(
  ...
  insertText(text2) {
    ...
    this.editor.chain().insertContent(text2).run();
  }
  getFormats() {
    if (!this.editor) {
      return null;
    }
    const linkAttrs = this.editor.getAttributes(EditorMarks.link);
    const link2 = linkAttrs?.href ? {
      href: String(linkAttrs.href),
      title: linkAttrs.title
    } : null;
    return {
      bold: this.editor.isActive(EditorMarks.bold),
      italic: this.editor.isActive(EditorMarks.italic),
      underline: this.editor.isActive(EditorMarks.underline),
      strikethrough: this.editor.isActive(EditorMarks.strike),
      link: link2
    };
  }
  getCursorRect() {
    ...
    const from2 = state.selection.from;
    const containerRect = this.editor.view.dom.getBoundingClientRect();
    try {
      const rect = view.coordsAtPos(from2);
      return {
        x: rect.left,
  ...
  applyMention(mention, uuid) {
    this.editor?.commands.applyDetectedSpecialWord({
      ruleId: "mention",
      content: mention,
      attrs: {
        href: `${MENTION_URL_PREFIX}${uuid}`
      }
    });
  }
  ...
  checkIsContentChanged() {
    ...
    const currentContentState = this.editor.view.state.doc;
    const isChanged = !currentContentState.eq(this.initialContentState);
    this.emit("initialContentChanged", isChanged);
  }
  ...
  unwireEditorEvents(editor) {
    ...
    editor.off("focus", this.handleFocus);
    editor.off("blur", this.handleBlur);
    const editable = editor.view.dom;
    editable.removeEventListener("focus", this.handleFocus);
    editable.removeEventListener("blur", this.handleBlur);
  }
  handleEditorUpdate = () => {
    const document2 = this.editor?.state.doc;
    if (!document2) {
      return;
    }
    const text2 = document2.textBetween(0, document2.content.size, "\n", "\n");
    this.emit("textChanged", text2);
    const formats = this.getFormats();
    this.emit("formatsChanged", formats);
    this.checkIsContentChanged();
    const content = this.getContent();
    this.emit("contentChanged", content);
  };
  handleSelectionUpdate = () => {
    const sel = this.getSelection();
    this.emit("selectionChanged", sel);
    const cursorRect = this.getCursorRect();
    this.emit("cursorRectChanged", cursorRect);
    const formats = this.getFormats();
    this.emit("formatsChanged", formats);
  };
  handleFocus = () => {
...
const EditorArgsComparators = {
  ...
  heightChanged: ([prev], [next2]) => {
    return prev === next2;
  },
  selectionChanged: ([prev], [next2]) => {
    return prev?.start === next2?.start && prev?.end === next2?.end;
  },
  formatsChanged: ([prev], [next2]) => {
    if (!prev || !next2) {
      return false;
    }
    return Object.keys(prev).every((key) => {
      const formatKey = key;
      return prev[formatKey] === next2[formatKey];
    });
  },
  focusChanged: ([prev], [next2]) => {
    return prev === next2;
  },
  mentionSuggested: ([prev], [next2]) => {
    return prev === next2;
  },
  hashtagSuggested: ([prev], [next2]) => {
    return prev === next2;
  },
  cursorRectChanged: ([prev], [next2]) => {
    return Object.keys(prev ?? {}).every((key) => {
      const formatKey = key;
      return prev?.[formatKey] === next2?.[formatKey];
    });
  },
  contentChanged: ([prev], [next2]) => {
    return prev === next2;
  },
  textCountChanged: ([prev], [next2]) => {
    return prev === next2;
  },
...
class EditorEventsObserver {
  ...
  emit(event, ...args) {
    const comparator = EditorArgsComparators[event];
    const prev = this.lastArgsByEvent.get(event);
    if (prev && comparator && comparator(prev, args)) {
      return;
    }
    this.lastArgsByEvent.set(event, args);
...
class EditorResizeObserver {
  ...
  setupResizeObserver(target) {
    this.resizeObserver = new ResizeObserver((entries) => {
      ...
      if (!rect) return;
      this.onHeightChange?.(rect.height);
    });
...
class EditorManager {
  ...
  init(container) {
    this.adapter?.onEvent(
      (event, ...args) => this.eventsObserver.emit(event, ...args)
    );
  ...
  setMaxLength(limit) {
    this.adapter.setMaxLength(limit);
  }
  on(event, callback) {
    return this.eventsObserver.on(event, callback);
  }
  onAnyEvent(handler) {
    return this.eventsObserver.onAnyEvent(handler);
  }
  toggleBold() {
  ...
  insertText(text2) {
    this.adapter.insertText(text2);
  }
  applyLink(href) {
    this.adapter?.applyLink(href);
  }
  setPlaceholder(placeholder) {
    this.adapter?.setPlaceholder(placeholder);
  }
  setContent(content) {
  ...
  applyMention(mention, href) {
    this.adapter?.applyMention(mention, href);
  }
  applyHashtag(hashtag) {
